Ran Slack-AL + Optim on GSBP, with aggressive penalty reduction.
...
for(x in 1:50) {
  ...
  write_json(out, glue("final_results/gsbp/slack_optim_aggressive/data/run_{x}_results.json"), digits=NA)
}
```