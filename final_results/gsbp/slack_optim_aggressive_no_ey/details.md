Ran Slack-AL + Optim on GSBP, with aggressive penalty reduction, and no resorting to EY.
...
for(x in 1:50) {
  ...
  write_json(out, glue("final_results/gsbp/slack_optim_aggressive_no_ey/data/run_{x}_results.json"), digits=NA)
}
```