...
# Ackley constraint one
ackley.c1 <- function(X) {
  if (is.null(dim(X))) X <- matrix(X, nrow=1)
  X <- -5.0 + 15.0 * X
  constraint <- rowSums(X) / 10.0
  return(constraint)
} 

# Ackley constraint two
ackley.c2 <- function(X) {
  if (is.null(dim(X))) X <- matrix(X, nrow=1)
  X <- -5.0 + 15.0 * X
  constraint <- (rowNorms(X, method="euclidean") - 5.0) / 10.0
  return(constraint)
}

ackley.objective <- function(X) {
  if (is.null(dim(X))) X <- matrix(X, nrow=1)
  X <- -5.0 + 15.0 * X
  exponent_1 <- -0.2 * sqrt((1 / 10.0) * rowSums(X^2))
  exponent_2 <- (1 / 10.0) * rowSums(cos(2.0 * pi * X))
  objective <- (-20.0 * exp(exponent_1) - exp(exponent_2) + 20.0 + exp(1.0)) / 10.0
  return(objective)
}

ackley.constraints <- function(x){
  return(cbind(ackley.c1(x), ackley.c2((x))))
}

ackleyprob <- function(X, known.only=FALSE)
{
  if(is.null(nrow(X))) X <- matrix(X, nrow=1)
  if(known.only) stop("known.only not supported for this example")
  f <- ackley.objective(X)
  C <- ackley.constraints(X)
  return(list(obj=f, c=cbind(C[,1], C[,2])))
}

## set bounding rectangle for adaptive sampling
B <- matrix(c(rep(0,10), rep(1,10)),ncol=2)
