Ran EI-AL with no slack on Lockwood problem, with no resorting to EY.
...
for(x in 1:20) {
  ...
  write_json(out, glue("../../final_results/lockwood/no_slack_no_ey/data/run_{x}_results.json"), digits=NA)
}
```