...
B <- matrix(c(rep(0,222), rep(2,222)), ncol=2)
...
for(x in 1:30) {
  ## run ALBO
  set.seed(42+x)
  out <- new_auglag(runmazda, B, Bscale=1, start=300, end=2500, slack=2, fhat=FALSE, lambda=0, urate=100, ncandf = ncandf)
  write_json(out, glue("../../../final_results/mazda/slack_optim_no_ey/data/run_{x}_results.json"), digits=NA)
}